...
else if (any(grepl("BM4", rownames(x))) && !any(grepl("PB2", rownames(x)))) {
...
## attempt 2 at the fxn (code copied from rds_analysis_single_Df - draft 2)
```{r}

classification_analysis_v2 <- function(x) {
# start the analysis
  ## TH cells
...
test_output_v2 <- lapply(subset_list, classification_analysis_v2)

# Combine the results into a single data frame
test_final_output_v2 <- do.call(rbind, test_output_v2)
...
```{r}
results_list <- lapply(data_list, classification_analysis_v2)
...
convert_to_numeric <- function(strings, total_th_count) {
...
# Convert the vector
numeric_values <- convert_to_numeric(strings, total_th_count)
